  ...
  return (
    <div className="flex h-full overflow-hidden bg-[var(--ak-color-bg-app)] relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full relative">
        ...
        {!isLeftOpen && (
          <button onClick={() => setIsLeftOpen(true)} className="fixed left-6 bottom-6 w-12 h-12 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-gray-400 hover:text-green-500 transition-all z-30"><ChevronRightIcon className="w-6 h-6" /></button>
        )}
        {!isRightOpen && (
          <button onClick={() => setIsRightOpen(true)} className="fixed right-6 bottom-6 w-12 h-12 bg-white rounded-full shadow-lg border border-gray-100 flex items-center justify-center text-gray-400 hover:text-green-500 transition-all z-30"><ChevronLeftIcon className="w-6 h-6" /></button>
        )}
      </div>
