  ...
  if (activities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[300px] text-center p-6 opacity-50">
        ...
        <p className="text-sm font-medium text-gray-500">Noch keine Aktivitäten</p>
        <p className="text-[11px] text-gray-400">Führe eine Aktion aus, um sie hier zu sehen.</p>
      </div>
    );
  }
      ...
      <div className="relative pl-4 space-y-6 before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-px before:bg-gray-100">
        {activities.map((activity) => (
          <div key={activity.id} className="relative group">
            <div className="absolute -left-[13px] top-1 w-2.5 h-2.5 rounded-full bg-white border-2 border-[var(--ak-color-accent)] z-10 group-hover:scale-125 transition-transform" />
