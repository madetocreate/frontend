...
export type TelegramView = 'overview' | 'chats' | 'configuration' | 'broadcasts' | 'setup'

import type { ComponentType } from 'react'

const VIEWS: { id: TelegramView; label: string; icon: ComponentType<{ className?: string }> }[] = [
  { id: 'overview', label: 'Übersicht', icon: ChartBarIcon },
  { id: 'chats', label: 'Chats', icon: ChatBubbleLeftRightIcon },
  { id: 'broadcasts', label: 'Broadcasts', icon: PaperAirplaneIcon },
  { id: 'configuration', label: 'Konfiguration', icon: Cog6ToothIcon },
]

type TelegramSidebarWidgetProps = {
  activeView: TelegramView
  onViewSelect: (view: TelegramView) => void
}

export function TelegramSidebarWidget({
  activeView,
  onViewSelect,
}: TelegramSidebarWidgetProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-transparent">
      ...
      <SidebarFooter 
        primaryAction={{
          label: 'Bot einrichten',
          icon: SparklesIcon,
          onClick: () => onViewSelect('setup')
        }}
      />
    </div>
  )
