...
export function DrawerHeader({ title, subtitle, status, leading, trailing, onClose, showHelp = true }: DrawerHeaderProps) {
  ...
  return (
    <div className="sticky top-0 z-20 flex flex-col bg-[var(--ak-glass-bg)] backdrop-blur-md">
...
export function DrawerCard({ 
  ...
  return (
    <div className={clsx(
      "rounded-[var(--ak-radius-lg)] border border-[var(--ak-color-border-subtle)] bg-[var(--ak-color-bg-elevated)] shadow-[var(--ak-shadow-sm)] overflow-hidden",
      className
    )}>
      {title && (
...
export function ShortcutChip({ label }: { label: string }) {
  return (
    <kbd className="inline-flex items-center justify-center min-w-[18px] h-[18px] rounded-[4px] border border-[var(--ak-color-border-subtle)] bg-[var(--ak-color-bg-surface)] px-1 text-[9px] font-bold font-mono uppercase text-[var(--ak-color-text-muted)] shadow-sm">
      {label}
    </kbd>
  )
}
...
export function ActionButton({ 
  ...
  const variantMap = {
    primary: 'bg-[var(--ak-color-accent)] text-white hover:opacity-90 shadow-[var(--ak-shadow-colored)] border-transparent',
  ...
  return (
    <button 
      ...
      disabled={loading}
      className={clsx(
        "group flex w-full items-center justify-between rounded-xl p-3 border transition-all duration-200 active:scale-[0.98]",
        variantMap[variant],
    ...
    >
      <div className="flex items-center gap-3 min-w-0">
        <div className={clsx(
          'flex h-8 w-8 items-center justify-center rounded-lg transition-all shrink-0 group-hover:scale-110',
          variant === 'primary' ? 'bg-white/20' : 'bg-[var(--ak-color-bg-surface-muted)] group-hover:bg-[var(--ak-color-accent)]/10 text-[var(--ak-color-accent-strong)]'
        )}>
          {icon}
        </div>
...
export function DrawerEmptyState({ 
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-6 text-center space-y-6">
      <div className="h-20 w-20 rounded-3xl bg-[var(--ak-color-bg-surface-muted)] flex items-center justify-center text-[var(--ak-color-text-muted)] shadow-inner">
        {icon}
      </div>
      <div className="space-y-2">
        <h3 className="text-base font-semibold text-[var(--ak-color-text-primary)]">{title}</h3>
        <p className="text-sm text-[var(--ak-color-text-secondary)] leading-relaxed max-w-[240px]">
          {description}
        </p>
      </div>
...
export function DrawerListRow({
  ...
  const accentMap = {
    ...
    inbox: 'hover:bg-[var(--ak-color-accent-inbox)]/5',
    growth: 'hover:bg-[var(--ak-color-accent-growth)]/5',
    documents: 'hover:bg-[var(--ak-color-accent-documents)]/5',
  ...
  return (
    <button
      onClick={onClick}
      className={clsx(
        'flex w-full items-center gap-3 rounded-[var(--ak-radius-lg)] px-3 py-2 text-left transition-all duration-200',
        selected
    ...
    <div className="sticky top-0 z-20 flex flex-col border-b border-[var(--ak-color-accent)] bg-[var(--ak-color-accent-soft)]/10 backdrop-blur-md">
      <div className="flex h-14 items-center justify-between px-4 gap-4">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--ak-color-accent)] text-white font-bold text-sm">
            {count}
          </div>
          <div className="flex flex-col">
            ...
            <button 
              onClick={onClear}
              className="text-[10px] font-bold text-[var(--ak-color-accent-strong)] uppercase tracking-tight hover:opacity-80 transition-all text-left"
            >
              Auswahl aufheben
            </button>
          </div>
