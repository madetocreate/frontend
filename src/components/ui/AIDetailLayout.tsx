...
const SmartSummaryWidget = ({ text }: { text: string }) => {
  return (
    <div className="mb-6 relative group">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500/20 to-purple-500/20 rounded-[var(--ak-radius-lg)] blur opacity-50 group-hover:opacity-75 transition duration-500"></div>
      <div className="relative apple-glass-enhanced p-5 rounded-[var(--ak-radius-lg)] border border-blue-100/50 dark:border-blue-900/30">
        <div className="flex items-center gap-2 mb-2 text-xs font-semibold uppercase tracking-wider text-blue-600 dark:text-blue-400">
...
const AIOutputArea = ({ 
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 10 }}
      className="mt-6 flex flex-col h-full min-h-[300px] apple-glass-enhanced rounded-[var(--ak-radius-xl)] border border-[var(--ak-color-accent)]/30 shadow-[var(--ak-shadow-strong)] overflow-hidden"
    >
      {/* Header */}
  ...
  return (
    <div className={clsx("h-full flex flex-col bg-[var(--ak-color-bg-app)]", className)}>
      ...
      <div className="flex-none px-6 py-5 border-b border-[var(--ak-color-border-subtle)] flex items-start justify-between bg-[var(--ak-glass-bg)] backdrop-blur-md sticky top-0 z-10">
        <div>
          <h2 className="apple-headline-3 text-[var(--ak-color-text-primary)]">{title}</h2>
      ...
      <div className="flex-1 overflow-y-auto ak-scrollbar">
        <div className="max-w-3xl mx-auto p-6 md:p-8 space-y-8">
          ...
          <section className="space-y-3">
            ...
            <div className="apple-card-enhanced text-[var(--ak-color-text-primary)]">
              {originalContent}
            </div>
          </section>
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    activeAction?.id === action.id
                      ? "bg-[var(--ak-color-accent)] text-white border-transparent shadow-[var(--ak-shadow-colored)] scale-105"
                      : "bg-[var(--ak-color-bg-surface)] text-[var(--ak-color-text-primary)] border-[var(--ak-color-border-subtle)] hover:border-[var(--ak-color-accent)] hover:shadow-[var(--ak-shadow-md)] hover:-translate-y-0.5"
                  )}
                >
                  {action.icon ? (
                    <span className={clsx(activeAction?.id === action.id ? "text-white" : "text-[var(--ak-color-accent)]")}>
                      {action.icon}
                    </span>
                  ) : (
                    <SparklesIcon className={clsx("w-4 h-4", activeAction?.id === action.id ? "text-white" : "text-[var(--ak-color-accent)]")} />
                  )}
                  {action.label}
                </button>
              ))}
