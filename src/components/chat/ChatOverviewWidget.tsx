...
export function ChatOverviewWidget() {
  return (
    ...
    >
      {/* Hero / Stats Card */}
      <DrawerCard className="bg-gradient-to-br from-indigo-50/10 to-purple-50/10 border-indigo-100/20">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <div className="text-[10px] text-green-600 font-bold">+12% heute</div>
          </div>
      ...
      <DrawerCard title="Modi & Fähigkeiten">
        <div className="space-y-2">
          {[
            { label: 'Code Expert', icon: <CodeBracketIcon className="w-4 h-4" />, color: 'text-blue-500' },
            { label: 'Creative Writer', icon: <PencilSquareIcon className="w-4 h-4" />, color: 'text-purple-500' },
            { label: 'Strategist', icon: <LightBulbIcon className="w-4 h-4" />, color: 'text-amber-500' },
          ].map((mode, i) => (
          ...
          {[
            ...
            <div key={i} className="flex items-center justify-between gap-3 group cursor-pointer">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-[var(--ak-color-bg-surface-muted)] flex items-center justify-center text-[var(--ak-color-text-muted)] group-hover:text-indigo-500 transition-colors">
