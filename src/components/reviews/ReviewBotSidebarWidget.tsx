...
export type ReviewBotView = 'overview' | 'reviews' | 'configuration' | 'setup'

import type { ComponentType } from 'react'

const VIEWS: { id: ReviewBotView; label: string; icon: ComponentType<{ className?: string }> }[] = [
  { id: 'overview', label: 'Übersicht', icon: ChartBarIcon },
  { id: 'reviews', label: 'Reviews', icon: StarIcon },
  { id: 'configuration', label: 'Konfiguration', icon: Cog6ToothIcon },
]

type ReviewBotSidebarWidgetProps = {
  activeView: ReviewBotView
  onViewSelect: (view: ReviewBotView) => void
}

export function ReviewBotSidebarWidget({
  activeView,
  onViewSelect,
}: ReviewBotSidebarWidgetProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-transparent">
      ...
      <SidebarFooter 
        primaryAction={{
          label: 'Reviews verwalten',
          icon: DocumentTextIcon,
          onClick: () => onViewSelect('reviews')
        }}
      />
    </div>
  )
