...
class CalendarService {
  ...
  private accountId = 'default' // TODO: Get from auth context
  ...
  async getEvents(days: number = 7): Promise<CalendarEvent[]> {
    const response = await fetch(`${this.baseUrl}/events?account_id=${this.accountId}&days=${days}`)
  ...
  async getDaySummary(date: Date): Promise<DaySummary> {
    ...
    const response = await fetch(`${this.baseUrl}/day/${dateStr}?account_id=${this.accountId}`)
  ...
  async createEvent(event: Partial<CalendarEvent>): Promise<CalendarEvent> {
    const response = await fetch(`${this.baseUrl}/events?account_id=${this.accountId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async updateEvent(eventId: string, updates: Partial<CalendarEvent>): Promise<CalendarEvent> {
    const response = await fetch(`${this.baseUrl}/events/${eventId}?account_id=${this.accountId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
